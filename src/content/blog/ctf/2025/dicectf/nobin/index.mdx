...
I also recommend getting rid of the 96 second throttle, as well.
...
We apparently have the histogram waiting to get sent off,
as well as a "report body" JSON containing a `aggregation_coordination_origin` field.
...
In any case, it seems we can only leak 3 bits of information at a time.
```js
...
For instance, if `0`, we'd compute `1`, as per the example above. Then, for `1`, we'd redo the work to find `1`,
then do the work to find the `4`th range within that.
...
Over where we call the worklet, we just call it as many times as needed.
```js
    const run = async (frame) => {
...
* With the `Select URL API`, worklets can read `sharedStorage` (or not) and return a numeric index.
  ...
  * `sharedStorage.selectUrl` can be provided up to 8 urls, which can hypothetically leak 8 bits of information (2^3).
...
* The worklet needs to store some state so that it knows which index to return.
  * For instance, should it return the index for range `4-7` or the index for the subsequent range: `6-7`?
